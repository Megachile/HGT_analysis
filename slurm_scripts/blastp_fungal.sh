#!/bin/bash
#SBATCH --job-name=blastp_fungal
#SBATCH --output=logs/blastp_fungal_%A_%a.out
#SBATCH --error=logs/blastp_fungal_%A_%a.err
...
if [ $SLURM_ARRAY_TASK_ID -eq 0 ]; then
    if [ ! -f temp/oak_query_chunks/chunk_0.faa ]; then
        ...
        awk 'BEGIN {n=0;c=0} /^>/ {n++; if(n%1000==1){c++; file="temp/oak_query_chunks/chunk_" int((n-1)/1000) ".faa$
    fi
fi

# Process chunk
chunk="temp/oak_query_chunks/chunk_${SLURM_ARRAY_TASK_ID}.faa"
outfile="blast_results/results_fungal_${SLURM_ARRAY_TASK_ID}.out"
...
if [ ! -f blast_dbs/fungal_plant_combined_db.pin ]; then
